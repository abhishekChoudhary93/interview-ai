...
| `problem` | Executor reference, OPENING PROTOCOL (system prompt DATA), ProblemPanel, debrief metadata | `{ title, brief, opening_prompt }`. `opening_prompt` is embedded verbatim inside the Executor's OPENING PROTOCOL block. |
...
| Rabbit-holed in one sub-topic for 4+ turns | Planner ignored `consecutive_probes_on_subtopic >= 3` rule | `eval_history[].current_subtopic` + `consecutive_probes_on_subtopic`. Confirm the THREAD DEPTH RULE block + RUNTIME STATE counter render. Could add a JS backstop forcing `PIVOT_ANGLE` at depth >= 4. |
...
| Interviewer seeded a scale fact (e.g. "How does your design handle 500,000 redirects/sec?") | Planner wrote a number from `scale_facts` into `recommended_focus` without the candidate asking | Inspect the rendered `recommended_focus`; confirm STEP 6 SCALE-FACT INJECTION CHECK fires. Could add a JS post-scan as a backstop. |
...
- Move overrides — the Planner's `move` is trusted; the JS only validates the enum and enforces the 45-min CLOSE floor.
...
- Vocabulary discipline (was the v3/v4 leak guard) — v5 trusts the Planner prompt's "Never name a component the candidate hasn't raised" line. If telemetry shows Planner drift, reintroduce a substrate scan as a v5 patch.
...
The JS substrate's job is: load config, route flags / probes by `section_id`, account for time per section, persist directives + contract + breadth + pace + verdict_trajectory for the next turn, enforce the 45-min CLOSE floor, and run the verdict caps in the debrief.
